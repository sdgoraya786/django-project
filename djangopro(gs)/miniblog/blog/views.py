...
def dashboard(request):
    ...
    if request.user.is_authenticated:
        if request.user.is_superuser == True:
            posts = Post.objects.all()  # for admin all posts
        else:
            posts = Post.objects.filter(user_id=request.user.id) # Current login user posts
        user = request.user
        data = {
            'posts': posts,
            'username': user,
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if fm.is_valid():
                ...
                post = Post(title=title, desc=desc, user_id=request.user.id)
                post.save()
...
def update_post(request,id):
    if request.user.is_authenticated:
        ...
        # Only User releated post can update or admin can update
        if pi.user_id == request.user.id or request.user.is_superuser == True:
            if request.method == 'POST':
